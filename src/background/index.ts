...
// Rate-limiting: track last request timestamp per message type
const lastRequestTime: Record<string, number> = {};
const RATE_LIMIT_MS = 500;

const isRateLimited = (type: string): boolean => {
  const now = Date.now();
  if (lastRequestTime[type] && now - lastRequestTime[type] < RATE_LIMIT_MS) return true;
  lastRequestTime[type] = now;
  return false;
};

chrome.runtime.onMessage.addListener((message: ExtensionMessage, _sender, sendResponse) => {
  // Rate-limit all mutation/query messages (not GET_TOKENS which is needed on load)
  if (message.type !== 'GET_TOKENS' && isRateLimited(message.type)) {
    sendResponse({ success: false, error: 'Too many requests, please wait' });
    return true;
  }
...
/**
 * Extracts only the needed tokens and metadata from the page.
 * Raw document.cookie and full innerHTML NEVER leave the page context.
 */
function getPageContext() {
  const cookie = document.cookie;
  const html = document.documentElement.innerHTML;

  // Extract userId from cookie (only c_user, nothing else)
  const userIdMatch = /c_user=(\d+)/.exec(cookie);
  const userId = userIdMatch ? userIdMatch[1] : '';

  // Extract security tokens from HTML
